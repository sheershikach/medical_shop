...
if(isset($_POST['update_medicine'])){
 ...
 if(move_uploaded_file($update_m_image_tmp_name, $update_m_image_folder)){
        $update_query = mysqli_query($conn, "UPDATE `medicines` SET name = '$update_m_name', price = '$update_m_price', available_quantity = '$update_m_quantity', mfg_date = '$update_m_mfg_date', exp_date = '$update_m_exp_date', company_name = '$update_m_company_name', image = '$update_m_image' WHERE id = '$update_m_id'");
        if($update_query){
...
<head>
   ...
   <style>
     body {
      ...
      .table-container {
         border-radius: 20px; /* Adding border-radius to create an attractive shape */
         overflow: hidden; /* Hiding overflow to ensure the border-radius is visible */
      }
      ...
      table {
         ...
         border-radius: 20px; /* Adding border-radius to match the container */
         overflow: hidden; /* Hiding overflow to ensure the border-radius is visible */
      }
   ...
   <section class="edit-form-container">
      ...
      ?>
            ...
            <form action="" method="post" enctype="multipart/form-data">
               ...
               <input type="number" min="0" class="box" required name="update_m_available_quantity" value="<?php echo $fetch_edit['available_quantity']; ?>">
               <input type="date" class="box" required name="update_m_mfg_date" value="<?php echo $fetch_edit['mfg_date']; ?>">
               <input type="date" class="box" required name="update_m_exp_date" value="<?php echo $fetch_edit['exp_date']; ?>">
               <input type="text" class="box" required name="update_m_company_name" value="<?php echo $fetch_edit['company_name']; ?>">
